...
from frontstage_api.controllers import case_controller, collection_exercise_controller, iac_controller, \
    party_controller, survey_controller


logger = wrap_logger(logging.getLogger(__name__))

enrolment_details = surveys_api.model('EnrolmentDetails', {
    'enrolment_code': fields.String(required=True),
})


@surveys_api.route('/add-survey/confirm-add-organisation-survey')
class ConfirmAddOrganisationSurvey(Resource):

    @staticmethod
    @auth.login_required
    @surveys_api.expect(enrolment_details, validate=True)
    def post():
        ...
        logger.info('Successfully retrieved organisation and survey data', business_party_id=business_party_id,
